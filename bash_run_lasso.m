%% Runs vanilla LASSO to predict per-gene regulators.
...
regulators=importdata(regfile); 
regulatorexp=regulators.data;
mids=load(sprintf('%s',mfile));

maxNumCompThreads

seed=12345678; % seed randomizer
...
%for m=1:1
    ...
    [lambda_corrs, lambda_total, rmse_total, lambda_regs, lambda_reg_freqs, lambda_fold_regs] = do_lasso_all_lambda(targets,regulators, lambdas, numcvs, regweight_fname);
    ...
    [rand_lambda_totals, rand_lambda_rmse, rand_lambda_reg_freqs] = eval_rand_regs(targets, regulators, lambdas, numreps, numcvs, seed, @do_lasso_all_lambda, rand_prefix);
    ...
    fid=fopen(rmsefile,'w');    % open RMSE file
    for j=1:size(lambdas,2)
        ...
        fprintf(fid, 'Lambda %.2f corr GREATER THAN random: pearson=%f, rand mu %f, rand SD %f, p=%f\n', lambdas(j), lambda_total{j}, mean(rand_lambda_totals{j}), std(rand_lambda_totals{j}), p);
        fprintf(fid, 'Lambda %.2f err LESS THAN random: RMSE=%f, rand mu %f, rand SD %f, p=%f\n', lambdas(j), rmse_total{j}, mean(rand_lambda_rmse{j}), std(rand_lambda_rmse{j}), pr);
    ...
    fclose(fid);    % close RMSE file
    
    % print out regulators for successful lambdas
    ...
    title(sprintf('Module %d (Lasso)', mids(m)));
    ...
    print(gcf,'-dpdf', '-r300', sprintf('%s/module%d_lasso_all_lambdas.pdf', outdir, mids(m)));
    hold off;
